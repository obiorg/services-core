 ...
 */
package org.obi.services.core.moka7;
...
public class S7 {
    ...
    // Returns a 32 bit unsigned value : from 0 to 4 294 967 295 (2^32-1)
    ...
    /**
     * Get Double Integer At will return a 32 bit signed value : from 0 to
     * 4294967295 (2^32-1)
     * <p>
     * Convenient method that read a buffer {@code byte[] Buffer} from a specied
     * position {@code int Pos} in little indian.
     * <ul><li>Result = Buffer[Pos];
     * <li>Result <<= 8; <li>Result += (Buffer[Pos + 1] & 0x0FF);
     * <li>Result <<= 8; <li>Result += (Buffer[Pos + 2] & 0x0FF);
     * <li>Result <<= 8; <li>Result += (Buffer[Pos + 3] & 0x0FF);
     * </ul>
     *
     * @param Buffer content to be read and convert
     * @param Pos position from which to read the buffer
